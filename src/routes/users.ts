    ...
    schema: {
      ...
      body: {
        ...
        properties: {
          ...
          username: { type: 'string', minLength: 3, maxLength: 20, description: 'Optional username' },
  ...
  fastify.patch<{ Body: UpdateProfileBody }>('/users/profile', {
    ...
    try {
      ...
      if (username) {
        ...
        if (existingUser && existingUser.username !== username && await isUsernameTaken(username)) {
          return reply.code(400).send({
            message: 'Username is already taken',
            error: 'Bad Request',
            statusCode: 400
